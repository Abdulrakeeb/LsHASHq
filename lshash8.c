/*
* This is the implementation of LsHASH8 algorithm by Abdulrakeeb Al-Ssulami
...
int search(unsigned char *x, int m, unsigned char *y, int n)
{
	...
	{
		Sj = (((((((((((((x[j-7]<<t)^x[j-6])<<t)^x[j-5])<<t)^x[j-4])<<t)^x[j-3])<<t)^x[j-2])<<t)^x[j-1])<<t)^x[j];
		H[Sj] = mMinus1 - j;
		B[Sj] = true;
	}
	Sj = (((((((((((((x[j-7]<<t)^x[j-6])<<t)^x[j-5])<<t)^x[j-4])<<t)^x[j-3])<<t)^x[j-2])<<t)^x[j-1])<<t)^x[j];
	...
	while (i < n)
	{
		Si = (((((((((((((y[i-7]<<t)^y[i-6])<<t)^y[i-5])<<t)^y[i-4])<<t)^y[i-3])<<t)^y[i-2])<<t)^y[i-1])<<t)^y[i];
		Sir = (((((((((((((y[i+mMinus14]<<t)^y[i+mMinus13])<<t)^y[i+mMinus12])<<t)^y[i+mMinus11])<<t)^y[i+mMinus10])<<t)^y[i+mMinus9])<<t)^y[i+mMinus8])<<t)^y[i+mMinus7];
		while (!(B[Si] || B[Sir]))
		{
			i = i + sh;
			Si = (((((((((((((y[i-7]<<t)^y[i-6])<<t)^y[i-5])<<t)^y[i-4])<<t)^y[i-3])<<t)^y[i-2])<<t)^y[i-1])<<t)^y[i];
