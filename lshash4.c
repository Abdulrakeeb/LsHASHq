/*
* This is the implementation of LsHASH4 algorithm by Abdulrakeeb Al-Ssulami
...
int search(unsigned char *x, int m, unsigned char *y, int n)
{
	...
	{
		Sj = (((((x[j - 3] << t) ^ x[j - 2]) << t) ^ x[j - 1]) << t) ^ x[j];
		H[Sj] = mMinus1 - j;
		B[Sj] = true;
	}
	Sj = (((((x[j - 3] << t) ^ x[j - 2]) << t) ^ x[j - 1]) << t) ^ x[j];
	...
	while (i < n)
	{
		Si = (((((y[i - 3] << t) ^ y[i - 2]) << t) ^ y[i - 1]) << t) ^ y[i];
		Sir = (((((y[i + mMinus6] << t) ^ y[i + mMinus5]) << t) ^ y[i + mMinus4]) << t) ^ y[i + mMinus3];
		while (!(B[Si] || B[Sir]))
		{
			i = i + sh;
			Si = (((((y[i - 3] << t) ^ y[i - 2]) << t) ^ y[i - 1]) << t) ^ y[i];
