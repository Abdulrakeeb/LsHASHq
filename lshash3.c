/*
* This is the implementation of LsHASH3 algorithm by Abdulrakeeb Al-Ssulami
...
int search(unsigned char *x, int m, unsigned char *y, int n)
{
	...
	while (i < n)
	{
		Si = (((y[i - 2] << t) ^ y[i - 1]) << t) ^ y[i];
		Sir = (((y[i + mMinus4] << t) ^ y[i + mMinus3]) << t) ^ y[i + mMinus2];
		while (!(B[Si] || B[Sir]))
		{
			i = i + sh;
			Si = (((y[i - 2] << t) ^ y[i - 1]) << t) ^ y[i];
